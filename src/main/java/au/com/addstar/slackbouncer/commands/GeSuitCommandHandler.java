...
public class GeSuitCommandHandler implements ISlackCommandHandler {
  ...
  @Override
  public void onCommand(SlackCommandSender sender, String command, String[] args)
  ...
  public void onSeen(SlackCommandSender sender, String[] args) {
    ...
    String seenInfo = ChatColor.translateAlternateColorCodes('&',
        PlayerManager.getLastSeeninfos(args[0], true, true));
  ...
  public void onWhere(final SlackCommandSender sender, String[] args) {
    ...
    if (search.contains(".")) {
      ...
      if (!DatabaseManager.players.playerExists(searchString)) {
        ...
        if (matches.isEmpty()) {
          ...
          attachment.setFormats(MarkDownFormats.builder().formatText(true).build());
      ...
      if (tracking.isEmpty()) {
        ...
        if (geSuit.proxy.getPlayer(searchString) != null) {
          ...
          if (location != null) {
            attachment.addField(
                new Attachment.AttachmentField("Location", location, false));
          }
        }
      }
    }
    ...
    attachment.setText(Joiner.on('\n').join(lines));
    attachment.setFormats(MarkDownFormats.builder().formatText(false).build());
  ...
  public void onWarnHistory(SlackCommandSender sender, String[] args) {
    ...
    if (target == null) {
      Map<String, UUID> ids =
          DatabaseManager.players.resolvePlayerNamesHistoric(Collections.singletonList(args[0]));
    ...
    for (Ban b : warns) {
      ...
      if (age >= ConfigManager.bans.WarningExpiryDays) {
        lines.add(String.format("expired %s - %s by %s", sdf.format(b.getBannedOn()),
            b.getReason(), b.getBannedBy()));
      } else {
        lines.add(String.format("%d: %s - %s by %s", ++count, sdf.format(b.getBannedOn()),
            b.getReason(), b.getBannedBy()));
      }
    }

    attachment.setText(Joiner.on('\n').join(lines));
    attachment.setFormats(MarkDownFormats.builder().build());
  ...
  public void onBanHistory(SlackCommandSender sender, String[] args) {
    ...
    String targetId;
    if (target == null) {
      Map<String, UUID> ids = DatabaseManager.players.resolvePlayerNamesHistoric(
    ...
    for (Ban b : bans) {
      ...
      if (b.getBannedUntil() == null) {
        lines.add(String.format("permban %s - %s by %s",
            sdf.format(b.getBannedOn()), b.getReason(), b.getBannedBy()));
      } else {
        lines.add(String.format("tempban %s - %s by %s until %s",
            sdf.format(b.getBannedOn()), b.getReason(), b.getBannedBy(),
            sdf.format(b.getBannedUntil())));
      }
    }
  ...
  public void onNames(SlackCommandSender sender, String[] args) {
    ...
    try {
      ...
      Map<String, UUID> result =
          DatabaseManager.players.resolvePlayerNamesHistoric(Collections.singletonList(nameOrId));
      if (result.isEmpty()) {
    ...
    attachment.setText(Joiner.on('\n').join(lines));
    MarkDownFormats formats = MarkDownFormats.builder().formatText(false).build();
    sender.sendMessage("Heres the results",
        MessageOptions.builder()
            .asUser(true)
            .attachments(Collections.singletonList(attachment))
